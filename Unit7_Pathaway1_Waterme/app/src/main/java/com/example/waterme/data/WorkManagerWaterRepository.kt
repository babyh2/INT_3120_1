...
class WorkManagerWaterRepository(context: Context) : WaterRepository {
    ...
    override fun scheduleReminder(duration: Long, unit: TimeUnit, plantName: String) {
        ...
        // Create a one-time work request with the given delay and input data
        val workRequest = OneTimeWorkRequestBuilder<WaterReminderWorker>()
            .setInitialDelay(duration, unit)
            .setInputData(data)
            .build()

        // Use a unique name composed of the plant name and the duration so multiple
        // reminders can be scheduled for the same plant with different delays.
